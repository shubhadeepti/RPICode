...
frame = Frame(canvas,width=800,height=600)
vsb = Scrollbar(root, orient="vertical", command=canvas.yview)
canvas.configure(yscrollcommand=vsb.set)

vsb.pack(side="right", fill="y")
canvas.pack(side="left", fill="both", expand=True)
canvas.create_window((30,30), window=frame, anchor=N+W)
...
def submit():
    array=[]
    array.append(int(v1.get()))
    array.append(int(v2.get()))
    array.append(v3.get())
    array.append(v4.get())
    array.append(v5.get())
    array.append(v6.get())
    ...
    print array
    with open('symptoms.csv','wb') as file:
	wr=csv.writer(file,quoting=csv.QUOTE_MINIMAL)
	wr.writerow(array)
    cmd='sudo python dhas3.py '+str(array)
    print(cmd)
