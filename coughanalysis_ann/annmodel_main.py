...
def disease_classify(path, patientID, symptoms):
    ...
    for feature in features_total:
        feature_vector = d = ",".join(str(x) for x in feature)
        try:
            cursor.execute("INSERT INTO va_patient_disease_features values(%s,%s)", (patientID, feature_vector))
        except:
...
if __name__ == '__main__':
    ...
    features=disease_classify(events_path, patientID, symptoms)

    cursor.close()  # close the cursor

    db.close()  # close the connection
    x,y=np.shape(features)
    json_data=dumps({"patientID":patientID,"features":x},primitives=True)
    print(json_data)
